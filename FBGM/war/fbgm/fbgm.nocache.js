...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "fbgm" because\n';
 ...
 * "/fbgm.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['fbgm'].superdevmode = true;

  var expectedSuffix = '/fbgm.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + fbgm + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:fbgm';
  var devModeSessionKey = '__gwtDevModeSession:fbgm';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/fbgm/fbgm.nocache.js';
  ...
  $wnd.__gwt_activeModules['fbgm'].canRedirect = true;
  $wnd.__gwt_activeModules['fbgm'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/fbgm?user.agent=' + ua + '&_callback=' + callback;
